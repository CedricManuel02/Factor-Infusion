...
function Infusion() {
  ...
  useEffect(() => {
    ...
    if(token_ || user.length > 0){
      const getData = async () => {
        try{
        const { data } = await axios.get("https://sleepy-seal-sari.cyclic.cloud/api/v1/list",{params: { UserID: user[0]._id  }, headers: { Authorization: `Bearer ${token_}` }})
        setList(data)
        setDefault(data)
        }
        catch(err){
  ...
  const handleDelete = async (_id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response = await axios.delete("https://sleepy-seal-sari.cyclic.cloud/api/v1/delete", {params:{ _id }, 
          headers: {
